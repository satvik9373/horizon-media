...
var ddbeforeandafter = (function($) {
    ...
    function ddbeforeandafter(options) {
        ...
        var dimensions = {
            ...
            dragbarw: $dragbar.width(),
            draghandlew: $draghandle.outerWidth()
        }
        ...
        $container.on(detecttouch ? 'touchstart' : 'click', function(e) {
            if (s._imagesloaded && (isdirectclick || detecttouch)) {
                ...
                $dragbar.add($before).css({
                    transitionTimingFunction: s.defaultTimingFunction,
                    transitionDuration: s.revealduration + 's'
                })
                $dragbar.css({
                    left: newx
                })
        ...
        $container.hover(
            ...
            function() {
                s._draghidetimer = setTimeout(function() {
                    $dragbar.stop().animate({
                        opacity: 0.3
                    })
                }, s.hidedragbardelay * 1000)
            }
        )
        ...
        $(window).bind('resize', function(e) {
            ...
            $before.find('img:eq(0)').css({
                width: dimensions.containerw,
                height: 'auto'
            })
            $after.find('img:eq(0)').css({
                width: dimensions.containerw,
                height: 'auto'
            })
    ...
    ddbeforeandafter.prototype = {
        ...
        _centerelement: function($parent, $target, offsetx, offsety) {
            ...
            $target.css({
                left: dimensions.parentw / 2 - dimensions.targetw / 2 + (offsetx || 0),
                top: dimensions.parenth / 2 - dimensions.targeth / 2 + (offsety || 0)
            })
        },
        ...
        _init: function(initoptions) {
            ...
            if (initoptions && initoptions.dimensions) {
                $container.css({
                    width: initoptions.dimensions[0],
                    height: initoptions.dimensions[1]
                })
                this.dimensions = {
                    containerw: $container.width(),
                    dragbarw: $dragbar.width(),
            ...
            var $beforeimage = this.$before.css({
                opacity: 0.5
            }).find('img:eq(0)').css({
                width: this.dimensions.containerw,
                height: 'auto'
            })
            var $afterimage = this.$after.css({
                opacity: 0
            }).find('img:eq(0)').css({
                width: this.dimensions.containerw,
                height: 'auto'
            ...
            $beforeimage.on('load error', function() {
                imagesloadeddfd[0].resolve()
            })
            $afterimage.on('load error', function() {
                imagesloadeddfd[1].resolve()
            })
        ...
        reload: function(beforeafterhtml, dimensions) {
            var options = {
                beforeafterhtml: beforeafterhtml,
